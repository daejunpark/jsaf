...
function writeTestCaseResult( expect, actual, string ) {
  ...
  if (passed) return writeFormattedResult( expect, actual, string, passed );
  else "FAILED (in writeTestCaseResult function)";
}
...
var gTestfile = 'with_unittest.js';
var UBound = 0;
var BUGNUMBER = '(none)';
...
var x = 50;
with ({ x : 2 }) { 
  var x, y, z;     // do nothing for rewriting
  var y=x+x, z=3, t;
  actual = y;
}
expect=4;
...
var x = 50;
with ({ x : 2 }) { 
  var ff=function(){   // we have nothing to rewrite in this function expression
    var y, z=2, x;
    z=10;
    return x;
  }
...
function test()
{
  ...
  {
    result += reportCompare(expectedvalues[i], actualvalues[i], statusitems[i]);
  }
