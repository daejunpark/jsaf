...
adsLo=adsLo||""
var adsUAC=adsLo.search(/atwUAC=/i),adsUACH,adsIntMN='';
...
s.src='http://o.aolcdn.com/os/moat/prod/moatuac.js';
h.appendChild(s); 
} 
}
function adsLoadUAC(){
...
adsVal='',adsCp=0,adsMNS,adsTPS,adsExcV='',adsLNm=0,adsKV,adsIP,
...
adsScr=(window.s_265&&window.s_265.prop55)?window.s_265.prop55:adsD.getTime()%0x3b9aca00,
adsDev=(typeof window.onorientationchange!='undefined')?'1':'2';
if ((adsUA.indexOf('mobile')>-1)||(/android|iphone|ipad|playbook|hp-tablet|kindle|silk|webos|blackberry/i).test(navigator.appVersion))adsDev='1';
if (!adsDev)adsDev='2';
...
return true
 ...
 if (!dv.dynSz)adsDevilObj(f.divName,f.w,f.h,f,d,'0');
 return false
}
}
...
if (z){ 
 adSetupDiv(f.w,f.h,z,f.divName,f.src,'text',f.mn);
...
adsDevilAd.ad[n]={
 ...
 height:h,
 mnum:m,
 aolDevilFlag:top.aol_devil_flag
};
...
var d=document,inc='',s,r=0,st="<script type='text/javascript' src='",sp1,ye=0,c=0,f=0,rr=0,wi=window;
...
if ((m.indexOf('-')>-1)&&(/^[0-9a-fm\-]+$/i.test(m))){
 ...
 adsMob=1;
 if (f)ds='r'
 var kf='kvmflash=',swh='',inI=false,inSD=true,pU;
 if (adsNMSG)kf+='true;'
 else kf+='false;';
 if (wi.screen && wi.screen.width && wi.screen.height)
 swh='swh='+wi.screen.width+'x'+wi.screen.height+';';
 try {
 ...
 s+=adsATOth+adsSrAT+adsATWM+adsVal+'kvmn='+m+';kvgrp='+adsScr+';kvismob='+adsDev+';'+adsChn+"extmirroring=0;"+adsTzAT+kf+swh+f1+f2+'random='+adsScr;
}
