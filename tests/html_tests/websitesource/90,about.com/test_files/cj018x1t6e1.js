...
mB=(!this.zD336||zD336==1)&&(g==336)&&(h==280)?3:1;ax='/'+bp+((td==uy)&&c.length?'abt.'+c:td)+'/'+c+'_'+gs+';'+zap+'svc='+s+';site='+(zgs.length?zgs:gs)+';t='+zTa+';bt='+zBT+';bts='+zBTS+zST(2,';st=')+';pc='+zpc+';auc='+(++zAUC)+';fd='+zFDT+';fs='+zFST+(this.zir?';r='+zir:'')+';sp2='+(zbXP2?1:0)+(zs>0?';go='+zGSk:'')+';a='+zc(0,'aut=')+tm+';kw='+k+';chan='+c+';syn=about;tile='+p+(mB==3?';af=0':'')+';r='+zRf+(bT?';dcopt=ist':'')+';sz='+d+';u='+zOr+';dc_ref='+escape(zWl)+';ord='+(bH?'1':o)+zOr
...
zSbL=i;w(x6)}
function zSae(n,d,i,u,t,r,b){x=xf+x1,y=x1+'>';w('<p onMouseOver="return zhl(1,'+n+')" onMouseOut="zhl(0)">'+x2+u+x1+qf+'"t" lnp="'+n+x3+d+x4+t+'</a>'+x2+u+x1+qf+'"u" lnp="'+n+x3+d+x4+r+'</a>'+x+'d'+y+d+xh+'</p>')}
...
if(t.indexOf('javascript:')!=0){zPxzT(c,50)}
return true}
...
else if(!s.f){zPxzT(zMu(s,z,window.event,1),50);return r!=0}}
