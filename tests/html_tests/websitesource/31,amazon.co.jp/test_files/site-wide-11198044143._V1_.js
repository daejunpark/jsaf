...
(function(window){if(window.$Nav&&!window.$Nav._replay){return;
}function argArray(args){return[].slice.call(args);
}var timestamp=Date.now||function(){return +(new Date());
};
var schedule=(function(){var toRun=[];
var scheduled;
function execute(){var i=0;
try{while(toRun[i]&&i<50){i++;
toRun[i-1].f.apply(window,toRun[i-1].a);
}}catch(e){var ePrefixed=e;
var msg="";
if(toRun[i-1].t){msg="["+toRun[i-1].t.join(":")+"] ";
}if(ePrefixed&&ePrefixed.message){ePrefixed.message=msg+ePrefixed.message;
}else{if(typeof e==="object"){ePrefixed.message=msg;
}else{ePrefixed=msg+ePrefixed;
}}if(window.ueLogError&&window.console&&window.console.error){window.ueLogError(ePrefixed);
window.console.error(ePrefixed);
}else{throw ePrefixed;
}}finally{toRun=toRun.slice(i);
if(toRun.length>0){setTimeout(execute,1);
}else{scheduled=false;
}}}return function(tags,func,args){toRun.push({t:tags,f:func,a:argArray(args||[])});
if(!scheduled){setTimeout(execute,1);
scheduled=true;
}};
}());
function ensureFunction(value){if(typeof value==="function"){return value;
}else{return function(){return value;
};
}}function extract(base,path){var parts=(path||"").split(".");
for(var i=0,len=parts.length;
i<len;
i++){if(base&&parts[i]){base=base[parts[i]];
}}return base;
}function withRetry(callback){var timeout=50;
var execute=function(){if(timeout<=20000&&!callback()){setTimeout(execute,timeout);
timeout=timeout*2;
}};
return execute;
}function isAuiP(){return(typeof window.P==="object"&&typeof window.P.when==="function"&&typeof window.P.register==="function"&&typeof window.P.execute==="function");
}function promise(){var observers=[];
var fired=false;
var value;
var trigger=function(){if(fired){return;
}fired=true;
value=arguments;
for(var i=0,len=observers.length;
i<len;
i++){schedule(observers[i].t,observers[i].f,value);
}observers=void (0);
};
trigger.watch=function(tags,callback){if(!callback){callback=tags;
tags=[];
}if(fired){schedule(tags,callback,value);
}else{observers.push({t:tags,f:callback});
}};
trigger.isFired=function(){return fired;
};
trigger.value=function(){return value||[];
};
return trigger;
}function promiseMap(){var promises={};
var getPromise=function(key){return promises[key]||(promises[key]=promise());
};
getPromise.whenAvailable=function(tags,dependencies){var onComplete=promise();
var length=dependencies.length;
if(length===0){onComplete();
return onComplete.watch;
}var fulfilled=0;
var observer=function(){fulfilled++;
if(fulfilled<length){return;
}var args=[];
for(var i=0;
i<length;
i++){args.push(getPromise(dependencies[i]).value()[0]);
}onComplete.apply(window,args);
};
for(var i=0;
i<length;
i++){getPromise(dependencies[i]).watch(tags,observer);
}return onComplete.watch;
};
return getPromise;
}function depend(options){options=options||{bubble:false};
var components=promiseMap();
var anonymousCount=0;
var debug={};
var iface={};
iface.declare=function(name,component){if(!debug[name]){debug[name]={done:timestamp()};
}components(name)(component);
};
iface.build=function(name,builder){schedule([options.sourceName,name],function(){iface.declare(name,ensureFunction(builder)());
});
};
iface.getNow=function(name,otherwise){return components(name).value()[0]||otherwise;
};
iface.publish=function(name,component){iface.declare(name,component);
var parent=options.parent;
if(options.prefix){name=options.prefix+"."+name;
}if(parent===(void 0)){if(window.amznJQ){window.amznJQ.declareAvailable(name);
}if(isAuiP()){window.P.register(name,function(){return component;
});
}}else{if(options.bubble&&parent.publish){parent.publish(name,component);
}else{if(parent.declare){parent.declare(name,component);
}}}};
iface.importEvent=function(name,opts){var parent=options.parent;
opts=opts||{};
var importAs=opts.as||name;
var whenReady=function(value){iface.declare(importAs,(value===(void 0)||value===null)?opts.otherwise:value);
};
if(parent&&parent.when&&parent.declare&&parent.build){parent.when(name).run(whenReady);
return;
}if(isAuiP()){window.P.when(name).execute(whenReady);
}if(window.amznJQ){var meth=options.useOnCompletion?"onCompletion":"available";
window.amznJQ[meth](opts.amznJQ||name,withRetry(function(){var value=opts.global?extract(window,opts.global):opts.otherwise;
var isUndef=(value===(void 0)||value===null);
if(opts.retry&&isUndef){return false;
}iface.declare(importAs,value);
return true;
}));
}};
iface.stats=function(unresolved){var result=JSON.parse(JSON.stringify(debug));
for(var key in result){if(result.hasOwnProperty(key)){var current=result[key];
...
for(var i=0;
i<depend.length;
i++){var dependency=depend[i];
if(!components(dependency).isFired()){blocked.push(dependency);
}else{if(debug[dependency].done>longPoleDone){longPoleDone=debug[dependency].done;
longPoleName=dependency;
}}}if(blocked.length>0){current.blocked=blocked;
}else{if(unresolved){delete result[key];
}else{if(longPoleDone>0){current.longPole=longPoleName;
current.afterLongPole=current.done-longPoleDone;
}}}}}return result;
};
iface.when=function(){var dependencies=argArray(arguments);
function onResolved(name,callback){var start=timestamp();
var tags=[options.sourceName,name];
debug[name]={depend:dependencies,registered:start};
components.whenAvailable(tags,dependencies)(tags,function(){debug[name].done=timestamp();
debug[name].wait=debug[name].done-start;
callback.apply(this,argArray(arguments));
});
}return{run:function(nameOrCallback,callbackWhenNamed){if(callbackWhenNamed){nameOrCallback="run."+nameOrCallback;
}else{callbackWhenNamed=nameOrCallback;
nameOrCallback="run.#"+anonymousCount++;
}onResolved(nameOrCallback,callbackWhenNamed);
},declare:function(name,value){onResolved(name,function(){iface.declare(name,value);
});
},publish:function(name,value){onResolved(name,function(){iface.publish(name,value);
});
},build:function(name,builder){onResolved(name,function(){var args=argArray(arguments);
var value=ensureFunction(builder).apply(this,args);
iface.declare(name,value);
});
}};
};
return iface;
}function make(sourceName){sourceName=sourceName||"unknownSource";
var instance=depend({sourceName:sourceName});
instance.declare("instance.sourceName",sourceName);
instance.importEvent("jQuery",{as:"$",global:"jQuery"});
instance.importEvent("jQuery",{global:"jQuery"});
instance.importEvent("amznJQ.AboveTheFold",{as:"page.ATF",useOnCompletion:true});
instance.importEvent("amznJQ.theFold",{as:"page.ATF",useOnCompletion:true});
instance.importEvent("amznJQ.criticalFeature",{as:"page.CF",useOnCompletion:true});
instance.when("$").run(function($){var triggerLoaded=function(){instance.declare("page.domReady");
instance.declare("page.ATF");
instance.declare("page.CF");
instance.declare("page.loaded");
};
$(function(){instance.declare("page.domReady");
});
$(window).load(triggerLoaded);
if(document.readyState==="complete"){triggerLoaded();
}else{if(document.readyState==="interactive"){instance.declare("page.domReady");
}}});
return instance;
}function callChain(obj,methods){for(var i=0,len=methods.length;
i<len;
i++){var invoke=methods[i];
if(!obj[invoke.m]){return;
}obj=obj[invoke.m].apply(obj,invoke.a);
}}function replaceShim(shim){var replacement=make(shim._sourceName);
if(!shim._replay){return;
}for(var i=0,iLen=shim._replay.length;
i<iLen;
i++){callChain(replacement,shim._replay[i]);
}for(var method in replacement){if(replacement.hasOwnProperty(method)){shim[method]=replacement[method];
}}delete shim._replay;
}function hookup(){if(!window.$Nav.make){window.$Nav.make=make;
return;
}if(!window.$Nav.make._shims){return;
}for(var i=0,len=window.$Nav.make._shims.length;
i<len;
i++){replaceShim(window.$Nav.make._shims[i]);
}window.$Nav.make=make;
}if(!window.$Nav){window.$Nav=make("rcx-nav");
}hookup();
...
(function(window,$Nav){$Nav.when("$").run(function($){$Nav.importEvent("legacy-popover",{as:"$popover",amznJQ:"popover",otherwise:$});
});
$Nav.importEvent("iss",{amznJQ:"search-js-autocomplete",global:"iss",retry:true});
if(!window.amznJQ){return;
}var amznJQ=window.amznJQ;
amznJQ.available("navbarInline",function(){$Nav.declare("logEvent",window._navbar.logEv);
...
amznJQ.available("navbarBTF",function(){$Nav.declare("config.flyoutURL",window._navbar.flyoutURL);
$Nav.declare("config.prefetch",window._navbar.prefetch);
});
$Nav.importEvent("navbarBTF",{as:"btf.full"});
$Nav.importEvent("navbarBTFLite",{as:"btf.lite"});
$Nav.importEvent("navbarInline",{as:"nav.inline"});
...
window.navbar={};
window.$Nav.when("depend").build("api.publish",function(depend){var $Nav=window.$Nav;
var apiComponents=depend({parent:$Nav,prefix:"api",bubble:false});
function use(component,callback){apiComponents.when(component).run(callback);
}window.navbar.use=use;
return function(name,value){apiComponents.publish(name,value);
window.navbar[name]=value;
$Nav.publish("nav."+name,value);
};
});
(function($Nav){$Nav.declare("throttle",function(wait,func){var throttling=false;
var called=false;
...
$Nav.when("$").build("agent",function($){return(new function(){function contains(){var args=Array.prototype.slice.call(arguments,0);
...
$Nav.declare("byID",function(elemID){return document.getElementById(elemID);
});
$Nav.build("dismissTooltip",function(){var dismissed=false;
return function(){if(dismissed){return;
}$Nav.publish("navDismissTooltip");
dismissed=true;
};
});
$Nav.build("recordEv",function(){var recordEvQueue=[],dups={};
$Nav.when("$","config.recordEvUrl","config.recordEvInterval","config.sessionId","config.requestId").run(function($,recordEvUrl,interval,sessionId,requestId){if(!recordEvUrl){return;
}function getSid(){var sid=window.ue&&window.ue.sid;
sid=sid||sessionId;
return sid?sid:"";
}function getRid(){var rid=window.ue&&window.ue.rid;
rid=rid||requestId;
return rid?rid:"";
}var cnt=0;
function send(when){cnt++;
if(recordEvQueue.length===0){return;
}when=when||cnt;
var msg=recordEvQueue.join(":");
msg=window.encodeURIComponent(msg);
msg="trigger="+msg+"&when="+when+"&sid="+getSid()+"&rid="+getRid();
var sep=recordEvUrl.indexOf("?")>0?"&":"?";
new Image().src=recordEvUrl+sep+msg;
recordEvQueue=[];
}window.setInterval(send,interval);
$(window).bind("beforeunload",function(){send("beforeunload");
});
});
...
$Nav.declare("TunableCallback",function(callback){var timeout=0;
var conditions=[];
...
$Nav.build("eachDescendant",function(){function eachDescendant(node,func){func(node);
node=node.firstChild;
while(node){eachDescendant(node,func);
node=node.nextSibling;
}}return eachDescendant;
});
$Nav.when("$","promise","TunableCallback").run("pageReady",function($,promise,TunableCallback){if(document.readyState==="complete"){$Nav.declare("page.ready");
return;
}var pageReady=promise();
function windowNotLoaded(){return document.readyState!="complete";
}function atfEnabled(){return !!$Nav.getNow("config.readyOnATF");
...
pageReady.watch([$Nav.getNow("instance.sourceName"),"pageReadyCallback"],function(){$Nav.declare("page.ready");
});
});
$Nav.when("$","agent").build("onOptionClick",function($,agent){return function(node,callback){var $node=$(node);
if((agent.mac&&agent.webkit)||(agent.touch&&!agent.ie10)){$node.change(function(){callback.apply($node);
});
...
if(time[primary]-time[secondary]<=100){callback.apply($node);
}};
};
$node.click(buildClickChangeHandler("click","change")).change(buildClickChangeHandler("change","click"));
}};
});
$Nav.when("$","promise","api.publish").build("triggerProceedToCheckout",function($,promise,publishAPI){var observers=promise();
publishAPI("onShowProceedToCheckout",function(callback){observers.watch([$Nav.getNow("instance.sourceName"),"onShowProceedToCheckoutCallback"],function(){var buttons=$("#nav_cart_flyout a[href~=proceedToCheckout]");
if(buttons.length>0){callback(buttons);
}});
});
return observers;
});
$Nav.when("promise","api.publish").run("triggerNearFlyout",function(promise,publishAPI){var observers=promise();
publishAPI("onNearFlyout",function(callback){observers.watch([$Nav.getNow("instance.sourceName"),"onNearFlyoutCallback"],callback);
});
$Nav.when("config.prefetchUrls","event.prefetch").run("prefetch",function(prefetchUrls,triggerNearFlyout){window.amznJQ&&window.amznJQ.addPL(prefetchUrls);
observers();
});
});
$Nav.when("$","byID").build("$byID",function($,byID){return function(elemID){return $(byID(elemID)||[]);
};
});
$Nav.when("$byID","config.dynamicMenuArgs","btf.full").build("flyout.primeAjax",function($id,dynamicMenuArgs){return dynamicMenuArgs.hasOwnProperty("isPrime")&&$id("nav-prime-menu").hasClass("nav-ajax-prime-menu");
});
$Nav.when("$","agent").build("areaMapper",function($,agent){var nullFn=function(){};
if(!agent.kindleFire){return{disable:nullFn,enable:nullFn};
}var disabled=$([]);
...
$Nav.when("$").build("template",function($){var cache={};
...
$Nav.when("$","config.yourAccountPrimeURL","nav.inline").run("yourAccountPrimer",function($,yourAccountPrimer){if(yourAccountPrimer){$("#nav_prefetch_yourorders").mousedown((function(){var fired=false;
return function(){if(fired){return;
}fired=true;
(new Image).src=yourAccountPrimer;
};
}()));
}});
$Nav.when("$","byID","agent","dismissTooltip","recordEv").build("flyout.NavButton",function($,byID,agent,dismissTooltip,recordEv){function NavButton(id){this._jqId="#"+id;
var button=this;
...
if(params.localContent){weblabs=$(params.localContent).attr("data-nav-wt");
...
if(agent.ie10touch){c.hoverShowDelay=250;
}return c;
};
}return NavButton;
});
$Nav.when("$byID","agent","$popover").build("flyout.SKIN",function($id,agent,$){return function(jqObject,xOffset,skinClass){var callback=function(){var navWidth=$id("nav-bar-outer").width();
...
if(agent.ie6){classes.push("nav_ie6");
...
$Nav.when("$","byID","eachDescendant").build("flyout.computeFlyoutHeight",function($,byID,eachDescendant){var flyoutHeightTable={};
return function(id){if(id in flyoutHeightTable){return flyoutHeightTable[id];
}var elem=byID(id);
...
}else{if(isShortDeep){height+=$node.hasClass("nav_pop_li")?28:0;
...
$Nav.when("$popover","$byID","agent","logEvent","recordEv","areaMapper","flyout.NavButton","flyout.SKIN","flyout.computeFlyoutHeight","flyout.initBrowsePromos","config.responsiveGW","config.sbd","nav.inline","flyout.content").run("ShopAll",function($,$id,agent,logEv,recordEv,areaMapper,NavButton,SKIN,computeFlyoutHeight,initBrowsePromos,responsiveGW,sbd_config){if(!$id("nav-shop-all-button").length||!$id("nav_browse_flyout").length){return;
}var nullFn=function(){};
...
}var onShow=function(){var initExposeSBD=$Nav.getNow("initExposeSBD");
if(initExposeSBD){initExposeSBD().deBorder(true);
...
var doCatChange=function(cat,args){var animateSubcat=!activeCat,resizeSubcat=false,$subcat=$id("nav_subcats_"+cat),$cat=$id("nav_cat_"+cat),promoID=$subcat.attr("data-nav-promo-id"),wtl=$subcat.attr("data-nav-wt");
...
if(promoID){var browsepromos=$Nav.getNow("config.browsePromos",{});
var imp={"t":"sa","id":promoID};
if(browsepromos[promoID]){imp["bp"]=1;
...
}}}if(isResponsive&&!agent.ie6){$subcat.parents(".nav_exposed_skin").removeClass("nav_exposed_skin");
...
if(initExposeSBD){initExposeSBD().deBorder(false);
}};
$Nav.declare("initExposeSBD",function(){if(exposeSBDData){return exposeSBDData;
...
$Nav.when("protectExposeSBD").run("exposeSBD",function(protectExposeSBD){protectExposeSBD(function(expose){lastState=expose;
...
var triggerParams={localContent:"#nav_browse_flyout",locationAlign:"left",locationOffset:[agent.ie6?3:0,0],skin:SKIN(jQButton,0),onShow:function(){var initExposeSBD=$Nav.getNow("initExposeSBD");
if(initExposeSBD){initExposeSBD().initDeferredShow();
}button.onShow();
...
button.onHide();
var initExposeSBD=$Nav.getNow("initExposeSBD");
if(initExposeSBD){initExposeSBD().deferredShow();
}}};
if(!isResponsive){button.registerTrigger(triggerParams);
}$Nav.declare("flyout.shopall");
});
$Nav.when("$","$byID","nav.inline","flyout.content").build("flyout.notificationCount",function($,$id){var notiCount=parseInt($("#nav-noti-wrapper .nav-noti-content").attr("data-noti-count")||"0",10);
var $count;
function notificationCount(count){notiCount=count;
if($count){if(notiCount<=0){$count.remove();
}else{$count.text(notiCount>9?"9+":notiCount);
}}}notificationCount.count=function(){return notiCount;
};
notificationCount.decrement=function(){notificationCount(notiCount-1);
};
$Nav.when("page.ready").run(function(){if(notiCount<=0){return;
...
var countLeft=positioner.position().left+10,$button=$id("nav-your-account"),tabWidth=$button.outerWidth()-5,freePixels=tabWidth-countLeft,cssObj={};
...
$Nav.when("$","$byID","agent","flyout.notificationCount","config.dismissNotificationUrl","flyout.content").build("flyout.notifications",function($,$byID,agent,notificationCount,dismissNotificationUrl){var $yaSidebarWrapper=$byID("nav-noti-wrapper");
var $yaSidebar=$yaSidebarWrapper.find(".nav-noti-content");
var $notiItems=$yaSidebar.find(".nav-noti-item").not("#nav-noti-empty");
function hideOverflow(){var cutoff=$yaSidebar.height()-$byID("nav-noti-all").outerHeight(true);
$notiItems.each(function(){var $this=$(this);
...
}function hookupBehaviour(){if(agent.touch){$notiItems.addClass("nav-noti-touch");
...
var $this=$(this);
$.ajax({url:dismissNotificationUrl,type:"POST",data:{"id":$this.attr("data-noti-id")},cache:false,timeout:500});
$this.css("visibility","hidden").parent().attr("data-dismissed","1").slideUp(400,function(){notificationCount.decrement();
if(notificationCount.count()===0){$byID("nav-noti-empty").fadeIn(300);
}hideOverflow();
});
}).hover(function(){$(this).addClass("nav-noti-x-hover");
},function(){$(this).removeClass("nav-noti-x-hover");
});
}return{width:180,exists:function(){return notificationCount.count()>0&&$yaSidebar.length>0;
},getContent:function(){var node=$yaSidebar.get(0);
node.parentNode.removeChild(node);
hookupBehaviour();
return $yaSidebar;
},onShow:hideOverflow,event:notificationCount.count()>0?"noti":null};
});
$Nav.when("$","flyout.content").build("flyout.highConfidence",function($){var hcb=$("#csr-hcb-wrapper .csr-hcb-content");
return{width:229,exists:function(){return hcb.length>0;
},getContent:function(){var node=hcb.get(0);
node.parentNode.removeChild(node);
return hcb;
},event:"hcb"};
});
$Nav.when("$","$byID","areaMapper","agent","logEvent","flyout.NavButton","flyout.SKIN","flyout.loadMenusConditionally","flyout.notifications","flyout.highConfidence","config.signOutText","nav.inline","flyout.content").run("YourAccount",function($,$id,areaMapper,agent,logEv,NavButton,SKIN,loadDynamicMenusConditionally,notifications,highConf,signOutText){var sidebar;
if(notifications.exists()){sidebar=notifications;
}else{if(highConf.exists()){sidebar=highConf;
...
if(sidebar&&sidebar.onShow){sidebar.onShow();
}areaMapper.disable("#nav_your_account_flyout");
delayImpression=window.setTimeout(function(){var imp={"t":"ya"};
if(sidebar&&sidebar.event){imp[sidebar.event]=1;
}logEv(imp);
},750);
if($yaSidebar){$yaSidebar.height($yaFlyout.height());
...
var sidebarOffset=0;
if(sidebar){sidebarOffset=sidebar.width+19;
$yaSidebar=sidebar.getContent();
...
button.registerTrigger({localContent:"#nav_your_account_flyout",locationAlign:leftAlignedYA?"left":"right",locationOffset:[YALocationOffsetLeft,0],skin:SKIN(jQButton,(leftAlignedYA||!agent.ie6)?0:7),onShow:onShow,onHide:onHide,followLink:!agent.touch});
if(signOutText){$("#nav-item-signout").html(signOutText);
}$Nav.declare("flyout.youraccount");
});
$Nav.when("$","$byID","areaMapper","agent","logEvent","triggerProceedToCheckout","flyout.NavButton","flyout.SKIN","flyout.loadMenusConditionally","nav.inline","flyout.content").run("Cart",function($,$id,areaMapper,agent,logEv,triggerProceedToCheckout,NavButton,SKIN,loadDynamicMenusConditionally){if(!$id("nav-cart").length||!$id("nav_cart_flyout").length){return;
...
button.registerTrigger({localContent:"#nav_cart_flyout",locationAlign:"right",locationOffset:[agent.ie6?-3:0,0],skin:SKIN(jQButton,agent.ie6?7:0),onShow:onShow,onHide:onHide,followLink:!agent.touch});
$Nav.declare("flyout.cart");
});
$Nav.when("$","$byID","areaMapper","agent","logEvent","flyout.NavButton","flyout.SKIN","flyout.loadMenusConditionally","nav.inline","flyout.content").run("WishList",function($,$id,areaMapper,agent,logEv,NavButton,SKIN,loadDynamicMenusConditionally){if(!$id("nav-wishlist").length||!$id("nav_wishlist_flyout").length){return;
...
button.registerTrigger({localContent:"#nav_wishlist_flyout",locationAlign:"right",locationOffset:[agent.ie6?-3:0,0],skin:SKIN(jQButton,agent.ie6?7:0),onShow:onShow,onHide:onHide,followLink:!agent.touch});
$Nav.declare("flyout.wishlist");
});
$Nav.when("$","$byID","flyout.NavButton","areaMapper","logEvent","agent","nav.inline","flyout.content").run("PrimeTooltip",function($,$id,NavButton,areaMapper,logEv,agent){if(!$id("nav-prime-ttt").length||!$id("nav-prime-tooltip").length){return;
...
$Nav.when("$byID","areaMapper","logEvent","agent","config.dynamicMenuArgs","flyout.NavButton","flyout.SKIN","flyout.primeAjax","flyout.loadMenusConditionally","nav.inline","flyout.content").run("PrimeMenu",function($id,areaMapper,logEv,agent,dynamicMenuArgs,NavButton,SKIN,primeAJAX,loadDynamicMenusConditionally){if(!$id("nav-your-prime").length||!$id("nav-prime-menu").length){return;
}var id="nav-your-prime",button=new NavButton(id),jQButton=$id(id),delayImpression;
if(primeAJAX){$id("nav-prime-menu").css({"width":dynamicMenuArgs.primeMenuWidth});
}var onShow=function(){button.onShow();
areaMapper.disable("#nav-prime-menu");
delayImpression=window.setTimeout(function(){logEv({"t":"prime"});
},750);
if(primeAJAX){loadDynamicMenusConditionally();
}};
var onHide=function(){clearTimeout(delayImpression);
areaMapper.enable();
button.onHide();
};
button.registerTrigger({localContent:"#nav-prime-menu",locationAlign:"right",locationOffset:[agent.ie6?-3:0,0],skin:SKIN(jQButton,agent.ie6?7:0),onShow:onShow,onHide:onHide,followLink:!agent.touch});
$Nav.declare("flyout.prime");
});
$Nav.when("$","byID","agent","api.publish","config.lightningDeals","nav.inline").run("UpdateAPI",function($,byID,agent,publishAPI,lightningDealsData){var navbarAPIError="no error";
publishAPI("error",function(){return navbarAPIError;
});
function update(navDataObject){var err="navbar.update() error: ";
if(navDataObject instanceof Object){$Nav.getNow("unlockDynamicMenus",function(){})();
var closures=[];
var cleanupClosures=[];
if(navDataObject.catsubnav){try{var navCatSubnav=$("#nav-subnav");
if(navCatSubnav.length>0){var svDigest=navDataObject.catsubnav.digest||"";
if(!svDigest||svDigest!==navCatSubnav.attr("data-digest")){var catSubnavChanges=0;
var newCatSubnav=[];
...
}if(catSubnavChanges==2){closures.push(function(){navCatSubnav.empty().append(newCatSubnav).css("display","").attr("data-digest",svDigest);
...
if(LDData){closures.push(function(){lightningDealsData=LDData;
});
}}}catch(e){navbarAPIError=err+e;
return false;
...
}if(!sddMeta.digest||sddMeta.digest!==dropdown.attr("data-nav-digest")){closures.push(function(){dropdown.blur().empty();
for(var i=0;
i<options.length;
...
}else{if(sddMeta.selected!=dropdown.attr("data-nav-selected")){closures.push(function(){dropdown.attr("data-nav-selected",sddMeta.selected).get(0).selectedIndex=sddMeta.selected;
$Nav.getNow("refreshDropDownFacade",function(){})();
});
}}}}catch(e){navbarAPIError=err+e;
return false;
...
publishAPI("setCartCount",function(newCartCount){return update({"cart":{"type":"count","data":{"count":newCartCount}}});
});
publishAPI("getLightningDealsData",function(){return lightningDealsData||{};
});
publishAPI("overrideCartButtonClick",function(clickHandler){if(!agent.touch){$("#nav-cart").click(clickHandler);
}$("#nav-cart-menu-button").click(clickHandler);
});
});
$Nav.when("$","$byID","agent","template","nav.inline").build("flyout.initBrowsePromos",function($,$id,agent,template){var initialized=false;
return function(){var browsepromos=$Nav.getNow("config.browsePromos");
if(!browsepromos||initialized){return;
}initialized=true;
$("#nav_browse_flyout .nav_browse_subcat").each(function(){var $this=$(this),promoID=$this.attr("data-nav-promo-id");
if(promoID){var data=browsepromos[promoID];
if(data){if(data.promoType){if(data.promoType=="wide"){$this.addClass("nav_super_cat");
}var mapID="nav_imgmap_"+promoID,vOffset=agent.ie6?15:14,bottom=parseInt(data.vertOffset,10)-vOffset,right=parseInt(data.horizOffset,10),ie6Hack=agent.ie6&&/\.png$/i.test(data.image),imgSrc=ie6Hack?$id("nav_trans_pixel").attr("src"):data.image,$img=$("<img>").attr({src:imgSrc,alt:data.alt,useMap:"#"+mapID}).addClass("nav_browse_promo").css({bottom:bottom,right:right,width:data.width,height:data.height});
$this.prepend($img);
if(ie6Hack){$img.get(0).style.filter="progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+data.image+"',sizingMethod='scale')";
}}else{$this.prepend(template("#nav-tpl-asin-promo",data));
}}}});
};
});
$Nav.when("$","agent","config.flyoutURL","btf.full").run("FlyoutContent",function($,agent,flyoutURL){window._navbar.flyoutContent=function(flyouts){var invisibleDiv=$("<div>").appendTo(document.body).hide().get(0);
var html="";
for(var flyout in flyouts){if(flyouts.hasOwnProperty(flyout)){html+=flyouts[flyout];
}}invisibleDiv.innerHTML=html;
$Nav.declare("flyout.content");
};
if(flyoutURL){var script=document.createElement("script");
script.setAttribute("type","text/javascript");
script.setAttribute("src",flyoutURL);
var head=document.head||document.getElementsByTagName("head")[0];
head.appendChild(script);
}else{$Nav.declare("flyout.content");
}});
$Nav.when("$","$byID","template","config.enableDynamicMenus","config.dynamicMenuUrl","config.dynamicMenuArgs","config.ajaxProximity","flyout.primeAjax","nav.inline").run("DynamicAjaxMenu",function($,$id,template,enableDynamicMenus,dynamicMenuUrl,dynamicMenuArgs,ajaxProximity,primeAJAX){var ajaxLoaded=!enableDynamicMenus,mouseHookTriggered=!enableDynamicMenus;
$Nav.declare("unlockDynamicMenus",function(){ajaxLoaded=false;
...
}function loadDynamicMenus(){$Nav.when("flyout.content").run("LoadDynamicMenu",function(){if(ajaxLock||ajaxLoaded){return;
}ajaxLock=true;
...
$.ajax({url:dynamicMenuUrl,data:dynamicMenuArgs,dataType:"json",cache:false,timeout:10000,complete:function(){allParents.removeClass("nav-ajax-loading");
$.AmazonPopover.updateBacking("navbar");
ajaxLock=false;
...
if(data.cartDataStatus){$Nav.getNow("api.setCartCount",function(){})(data.cartCount);
...
}function loadDynamicMenusConditionally(){if(!mouseHookTriggered){loadDynamicMenus();
}mouseHookTriggered=true;
if(!preloadStarted){preloadStarted=true;
$Nav.declare("event.prefetch");
}return true;
...
$.AmazonPopover.mouseTracker.add([[left,top,width,height]],{inside:false,mouseEnter:loadDynamicMenusConditionally,mouseLeave:function(){return true;
}});
$("#nav_wishlist_flyout, #nav_cart_flyout").find(".nav-try-again").click(loadDynamicMenus);
}if(dynamicMenuUrl){$Nav.when("page.ready").run(attachTriggers);
}});
$Nav.when("$","onOptionClick","throttle","byID","$byID","btf.lite","nav.inline").run("SearchDropdown",function($,onOptionClick,throttle,byID,$id){var dropdownNode=byID("searchDropdownBox"),dropdown=$(dropdownNode),facade=$id("nav-search-in"),facadeContent=byID("nav-search-in-content"),searchBox=$id("twotabsearchtextbox"),searchBoxParent=(function(elem){if(elem.hasClass("nav-searchfield-width")){return elem;
...
if($.browser.msie){if(parseFloat($.browser.version)<7){$Nav.declare("refreshDropDownFacade",function(){});
return;
}facade.addClass("ie");
...
}function focusSearchBox(){var iss=$Nav.getNow("iss");
if(iss){state["isFocus"]=false;
iss.focus();
...
updateFacade();
dropdown.change(updateFacade).keyup(keypressHandler).focus(buildCallback("isFocus",true)).blur(buildCallback("isFocus",false)).hover(buildCallback("isHover",true),buildCallback("isHover",false));
$Nav.when("dismissTooltip").run(function(dismissTooltip){dropdown.focus(dismissTooltip);
});
onOptionClick(dropdown,function(){focusSearchBox();
updateFacade();
});
$(window).resize(throttle(150,updateFacadeWidth));
$Nav.when("page.ready").run("FixSearchDropdown",function(){if($(facadeContent).outerWidth(true)-facade.outerWidth()>=4||useAbbrDropdown()){updateFacadeWidth();
}tweakDropdownWidth();
dropdown.css({"top":Math.max(0,(facade.outerHeight()-dropdown.outerHeight())/2)});
});
$Nav.when("iss").run(function(iss){iss.keydown(function(e){setTimeout(function(){keypressHandler(e);
},10);
});
$Nav.when("dismissTooltip").run(function(dismissTooltip){iss.keydown(dismissTooltip);
});
...
$Nav.when("$").build("Keycode",function($){function Keycode(evt){this.evt=evt;
this.code=evt.keyCode;
}Keycode.prototype.isAugmented=function(){return this.evt.altKey||this.evt.ctrlKey||this.evt.metaKey;
};
Keycode.prototype.isAugmentor=function(){return 0<=$.inArray(this.code,[0,16,20,17,18,224,91,93]);
};
Keycode.prototype.isTextFieldControl=function(){return 0<=$.inArray(this.code,[8,9,13,32,35,36,37,38,39,40,45,46]);
};
Keycode.prototype.isControl=function(){if(this.code<=46){return true;
}else{if(this.code>=91&&this.code<=95){return true;
}else{if(this.code>=112&&this.code<=145){return true;
}}}return false;
};
Keycode.prototype.isTab=function(){return this.code===9;
};
Keycode.prototype.isEnter=function(){return this.code===13;
};
Keycode.prototype.isBackspace=function(){return this.code===8;
};
return Keycode;
});
$Nav.when("$","agent","iss","Keycode","config.autoFocus","nav.inline").run("autoFocus",function($,agent,iss,Keycode,enableAutoFocus){if(!enableAutoFocus){return;
}if(agent.touch){return;
}function getSelection(){if(window.getSelection){return window.getSelection().toString();
}else{if(document.selection){return document.selection.createRange().text;
}else{return"";
}}}function canFocus(){return $(document).scrollTop()<=$("#nav-iss-attach").offset().top&&$(document.activeElement).filter("input,select,textarea").size()<1&&getSelection()==="";
}var first=false;
if(canFocus()){iss.focus();
first=true;
}iss.keydown(function(e){var key=new Keycode(e);
if(key.isAugmentor()){return;
}var isControl=key.isControl();
if(key.isAugmented()){"noop";
}else{if(first){if(!canFocus()){iss.blur();
}else{if(isControl){iss.blur();
}}}else{if(isControl){if(!key.isTextFieldControl()){iss.blur();
}else{if(iss.keyword()===""&&!key.isTab()&&!key.isEnter()&&!key.isBackspace()){iss.blur();
}}}}}first=false;
});
$(document).keydown(function(e){var key=new Keycode(e);
if(!canFocus()){return;
}if(key.isControl()){return;
}if(key.isAugmentor()||key.isAugmented()){return;
}iss.focus();
});
});
$Nav.when("$","api.publish","config.swmStyleData").run("ExternalAPI",function($,publishAPI,swmStyleData){publishAPI("unHideSWM",function(){var $h=$("#navHiddenSwm");
var s=swmStyleData;
...
var exposeState;
publishAPI("exposeSBD",function(expose){exposeState=expose;
$Nav.when("initExposeSBD","protectExposeSBD").run(function(initExposeSBD,protectExposeSBD){if(initExposeSBD().ok()){protectExposeSBD(exposeState);
}});
});
publishAPI("navDimensions",function(){var elem=$("#navbar");
...
}if(window.$SearchJS){$SearchJS.importEvent("legacy-popover",{as:"popover",amznJQ:"popover",global:"jQuery.AmazonPopover"});
$SearchJS.when("jQuery","popover").run(function($,AmazonPopover){$.fn.amznFlyoutIntent=function(arg){var defaults={getTarget:function(el){return $(this).children('*[position="absolute"]').eq(0);
