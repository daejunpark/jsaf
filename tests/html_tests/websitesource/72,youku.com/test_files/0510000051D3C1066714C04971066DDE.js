...
(function(){
	...
	TUDO.link = {
		...
		getPositionCode : function(element) {
			...
			if (startIndex) {
				paths.splice(0, startIndex + 1, ids[startIndex]);
			}
...
TUDO[ 'clickStat' ] = {
	...
	getStatTarget : function( p ){
		var inputs;
		var check = {'A': 1, 'INPUT': 1, 'BUTTON': 1, 'TEXTAREA': 1};
	    // label包含input时，发送input点击统计
		if (p.nodeName == 'LABEL') {
		...
		// 当前节点或父节点包含data-stat-role="ck"属性
	...
	request: function(data){
		var eventId = 'eid=' + data.eventId,
			...
			channelId = 'cid=' + ((typeof(catId) != 'undefined') ? catId : 0 ),
...
window['nova_init_hook_TUDO'] = function(){ TUDO.init();};
