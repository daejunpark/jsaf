//gettting TZDate object
try {
	var dateObject = webapis.time.getCurrentDateTime();		
} catch (e) {
	con.log("exception while invoking getCurrentDateTime" + e.name + e.message);
}
//gettting TZDate object
try {
	var dateObjectforUTC = webapis.time.getCurrentDateTime();		
} catch (e) {
	con.log("exception while invoking getCurrentDateTime" + e.name + e.message);
}
...
function getCurrentDateTime() {	 
	...
	try {
		var current_date = webapis.time.getCurrentDateTime();		
	} catch (e) {
		con.log("exception while invoking getCurrentDateTime" + e.name + e.message);
	}
...
function getLocalTimezone() {	 
	...
	try {
		var localTimezone = webapis.time.getLocalTimezone();		
	} catch (e) {
...
function getAvailableTimezones() {	 
	...
	try {
		var availTimezones = webapis.time.getAvailableTimezones();		
	} catch (e) {
...
function getDateFormat() {	 
	...
	try {
		var shortformat = webapis.time.getDateFormat(true);
		var longformat = webapis.time.getDateFormat(false);
	} catch (e) {
...
function getTimeFormat() {	 
	...
	try {
		var timeformat = webapis.time.getTimeFormat();
	} catch (e) {
...
function isLeapYear() {	 
	...
	try {
		var isleap = webapis.time.isLeapYear(2016);
	} catch (e) {
...
function difference() {	 
	...
	var diff_TZDate = new webapis.TZDate(2012, 11, 25, 10, 24, 5, 354, "Asia/Hong_Kong");
	var duration;
	try {
...
function addDuration() {	 
	...
	try{
		var duration = new webapis.TimeDuration(1, "DAYS");
	} catch(e){
...
function test() {
	alert("here");
	var testingTZDateObj = new webapis.TZDate(2012, 1, 1, 1, 1, 1, 1);
	...
	// param null
	testingTZDateObj = new webapis.TZDate(2012, 1, 1, 1, 1, 1, 1);
...
/*
	...
	try
	{
		testingTZDateObj.setFullYear({"a":"b"}); 
	}
	catch (e)
	{
		con.log("setFullYear Error"+e.name+e.message);
	...
	if (testingTZDateObj.getFullYear() == 1)
	{
		con.log('converstion from object to long success');
	} else {
		con.log('setFullYear Error object '+e.name+e.message);
	}
	*/
	// param numeric elt
	testingTZDateObj = new webapis.TZDate(2012, 1, 1, 1, 1, 1, 1);
