...
conditionalExpressionNoIn
	: logicalORExpressionNoIn (LT!* '?' LT!* assignmentExpression LT!* ':' LT!* assignmentExpressionNoIn)?
	;
...
// Another possibility
// '[' ((LT!* ',')* LT!* assignmentExpression ((LT!* ',')+ LT!* assignmentExpression)*)? (LT!* ',')* LT!* ']'
arrayLiteral
    : '[' (LT!* ',')* (LT!* assignmentExpression ((LT!* ',')+ LT!* assignmentExpression)* (LT!* ',')*)? LT!* ']'
	;

objectLiteral
	: '{' (LT!* propertyAssignment (LT!* ',' LT!* propertyAssignment)*)? (LT!* ',')? LT!* '}'
	;
...
LineTerminatorSequence
 ...
 // Tab, vertical tab, form feed, space, non-breaking space and any other unicode "space separator".
/* Not used
WhiteSpace
	: ('\t' | '\v' | '\f' | ' ' | '\u00A0')	{$channel=HIDDEN;}
	;
