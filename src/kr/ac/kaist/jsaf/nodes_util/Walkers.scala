...
trait Walkers {
  ...
  def walkCFG(parent: Any, cfg: Any): Unit = cfg match {
    ...
    case CFGConstruct(iid, info, cons, thisArg, arguments, addr) => walkCFG(cfg, cons); walkCFG(cfg, thisArg); walkCFG(cfg, arguments)
    case CFGCall(iid, info, fun, thisArg, arguments, addr) => walkCFG(cfg, fun); walkCFG(cfg, thisArg); walkCFG(cfg, arguments)
