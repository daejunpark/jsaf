...
import kr.ac.kaist.jsaf.analysis.typing.GSparseTyping;
...
import kr.ac.kaist.jsaf.analysis.typing.models.DOMModel;
...
public final class Shell {
    ...
    private static void printUsageMessage() {
        System.err.print(
            ...
            " cfg [-out file] [-test] [-model] somefile.js ...\n" +
        ...
        if (params.opt_OutFileName != null){
            try {
                ...
                return reportErrors(NodeUtil.getFileName(pgm),
                                    flattenErrors(errors), Option.<BufferedWriter>some(writer));
            } catch (IOException e){
                throw new IOException("IOException " + e +
                                      "while writing " + params.opt_OutFileName);
                }
            ...
            return reportErrors(NodeUtil.getFileName(pgm),
                                flattenErrors(errors), Option.<BufferedWriter>none());
        }
    }
    ...
    private static int analyze() throws UserError, InterruptedException, IOException {
        ...
        if(params.opt_Verbose) Config.setVerbose(new Boolean(params.opt_Verbose));
    ...
    private static int reportErrors(String file_name, List<? extends StaticError> errors,
                                    Option<BufferedWriter> writer) throws IOException {
        int return_code = 0;
        if (!IterUtil.isEmpty(errors)) {
            for (StaticError error: IterUtil.sort(errors)) {
                if (writer.isSome()) writer.unwrap().write(error.getMessage());
            ...
            if (writer.isSome()) writer.unwrap().write(err_string);
            else System.out.println(err_string);
            return_code = -2;
        }
        if (writer.isSome()) writer.unwrap().close();
        return return_code;
    }
