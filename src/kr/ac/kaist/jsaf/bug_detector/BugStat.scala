...
class BugStat(lib: Boolean) {
  ...
  private def printBugStatistics() = {
    ...
    System.out.println("|  BuiltinArgSize          : %6d |".format((BugCount(BuiltinArgSize))))
    System.out.println("|  BuiltinWrongType        : %6d |".format(BugCount(BuiltinWrongType)))
    ...
    System.out.println("|  AccessingNullOrUndef    : %6d |".format((BugCount(ObjectNull) + BugCount(ObjectNullOrUndef) + BugCount(ObjectUndef))))
