...
object PreSemanticsBuiltinString {
  ...
  def builtinString(sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG,
    ...
    fun match {
      case "String" => {
        ...
        val s = n_arglen match {
          case UIntSingle(n) if n == 0 =>
            AbsString.alpha("")
          case UIntSingle(n) if n > 0 =>
            PreHelper.toString(PreHelper.toPrimitive(getArgValue(h, ctx, "0")))
          case NumBot => StrBot
          case _ => StrTop
        }
        if (s </ StrBot)
        ...
        val s = n_arglen match {
          ...
          case UIntSingle(n) if n > 0 =>
            (0 until n.toInt).foldLeft(s_this)((_s, i) =>
              _s.concat(PreHelper.toString(PreHelper.toPrimitive((getArgValue(h, ctx, i.toString))))))
          case NumBot => StrBot
          case _ => StrTop
        }
      ...
      case "String.prototype.indexOf" => {
        ...
        val s_search = PreHelper.toString(PreHelper.toPrimitive(getArgValue(h, ctx, "0")))
      ...
      case "String.prototype.localeCompare" => {
        ...
        val v_this = lset_prim.foldLeft(ValueBot)((_v, l) => _v + h(l)("@primitive")._1._2)
        val s_this = PreHelper.toString(PreHelper.toPrimitive(v_this))
        val s_that = PreHelper.toString(PreHelper.toPrimitive(getArgValue(h, ctx, "0")))
