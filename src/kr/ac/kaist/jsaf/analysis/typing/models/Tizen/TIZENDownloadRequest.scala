...
object TIZENDownloadRequest extends Tizen {
  ...
  private val prop_cons: List[(String, AbsProperty)] = List(
    ...
    ("url", AbsConstValue(PropValue(Value(UndefTop)))),
    ("destination", AbsConstValue(PropValue(Value(UndefTop)))),
    ("fileName", AbsConstValue(PropValue(Value(UndefTop))))
  )
          ...
          val (h_3, es) = n_arglen match {
            ...
            case UIntSingle(n) if n == 4 =>
              ...
              val o_new3 = o_new2.
                ...
                update("fileName", PropValue(ObjectValue(Value(Helper.toString(v_3._1)), T, T, T))).
                update("httpHeader", PropValue(ObjectValue(Value(l_r1), T, T, T)))
              val h_4 = h_3.update(l_r2, o_new3)
              (h_4, TizenHelper.TizenExceptionBot)
            case UIntSingle(n) if n >= 5 =>
              ...
              val h_3 = h_2.update(l_r2, o_new3)
              (h_3, es1)
            case _ => {
