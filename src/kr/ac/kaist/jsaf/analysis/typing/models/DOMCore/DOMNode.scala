  ...
  def getSemanticMap(): Map[String, SemanticFun] = {
    Map(
          ...
          if (!lset_new.isEmpty && !lset_ref.isEmpty) {
            ...
            val h_1 = lset_this.foldLeft(h)((hh, l_node) => {
              ...
              lset_ns.foldLeft(hh)((hhh, l_ns) => {
                ...
                n_len match {
                  case UIntSingle(n_length) =>
                    val n_index = (0 until n_length.toInt).indexWhere((i) => {
                      BoolTrue <= Operator.bopSEq(Helper.Proto(hhh, l_ns, AbsString.alpha(i.toString)), Value(lset_ref))._1._3
                    })
                    if (n_index < 0)
                      hhh
                    else {
                      ...
                      Helper.PropStore(_hhh_1, l_ns, AbsString.alpha("length"), Value(AbsNumber.alpha(n_length + 1)))
                    }
                  case NumTop | UInt =>
                    val b_eq = Operator.bopSEq(Helper.Proto(hhh, l_ns, NumStr), Value(lset_ref))._1._3
                    val hhh_1 =
                      if (BoolTrue <= b_eq) Helper.PropStore(hhh, l_ns, NumStr, Value(lset_new))
                      else HeapBot
                    val hhh_2 =
        ...
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val b_return = lset_this.foldLeft[AbsBool](BoolBot)((b, l) => {
            ...
            lset_child.foldLeft(b)((bb, ll) => {
              bb + (Helper.Proto(h, ll, AbsString.alpha("length"))._1._4 match {
                case NumBot => BoolBot
                case NumTop | UInt => BoolTop
                case UIntSingle(n) => if (n != 0) BoolTrue else BoolFalse
                case NUIntSingle(n) => if (n != 0) BoolTrue else BoolFalse
                case NUInt | NaN | NegInf | PosInf | Infinity => BoolFalse
              })
            })
          })
          if (b_return </ BoolBot)
      ...
      ("DOMNode.isSupported" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          /* arguments */
          val s_feature = Helper.toString(Helper.toPrimitive(getArgValue(h, ctx, args, "0")))
          val s_version = Helper.toString(Helper.toPrimitive(getArgValue(h, ctx, args, "1")))
      ...
      ("DOMNode.lookupPrefix" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          /* arguments */
          val s_uri = Helper.toString(Helper.toPrimitive(getArgValue(h, ctx, args, "0")))
          if (s_uri </ StrBot)
          /* imprecise semantic */
      ...
      ("DOMNode.lookupNamespaceURI" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          /* arguments */
          val s_prefix = Helper.toString(Helper.toPrimitive(getArgValue(h, ctx, args, "0")))
          if (s_prefix </ StrBot)
          /* imprecise semantic */
            ((Helper.ReturnStore(h, Value(StrTop) + Value(NullTop)), ctx), (he, ctxe))
          else
      ...
      ("DOMNode.setUserData" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          /* arguments */
          val s_key = Helper.toString(Helper.toPrimitive(getArgValue(h, ctx, args, "0")))
      ...
      ("DOMNode.hasChildNodes" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val b_return = lset_this.foldLeft[AbsBool](BoolBot)((b, l) => {
            ...
            lset_child.foldLeft(b)((bb, ll) => {
              bb + (PreHelper.Proto(h, ll, AbsString.alpha("length"))._1._4 match {
                case NumBot => BoolBot
                case NumTop | UInt => BoolTop
                case UIntSingle(n) => if (n != 0) BoolTrue else BoolFalse
                case NUIntSingle(n) => if (n != 0) BoolTrue else BoolFalse
        ...
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val LP2 = lset_this.foldLeft(LPBot)((lpset, l) => {
            ...
            prop_set.foldLeft(lpset)((lpset1, prop) =>
              lpset1  ++ set_addr.foldLeft(LPBot)((lp, a) => lp + (addrToLoc(cfg.getAPIAddress(a, 0),Recent), prop)))
          })
