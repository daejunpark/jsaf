...
object HTMLParagraphElement {
  ...
  val loc_proto = newLoc("HTMLParagraphProto")
  
  val F = BoolFalse
  val T = BoolTrue
  
  def init(map: HeapMap) : HeapMap = {
    val loc_cons = newLoc("HTMLParagraphConst")
    ...
    val global_object = map(GlobalLoc).update(AbsString.alpha("HTMLParagraphElement"), 
  ...
  def instantiate(map: HeapMap, node: Node) : (HeapMap, Loc) = {
    val loc_instance = newLoc("HTMLParagraphInstance")
    ...
    // DOM Level 1
      update(OtherStrSingle("align"),   PropValue(ObjectValue(AbsString.alpha(elementnode.getAttribute("align")), T, T, T)))
