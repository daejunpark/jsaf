...
object JQueryMiscsllaneous extends ModelData {
    ...
    Map(
      ...
      ("jQuery.prototype.toArray" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h_ret, ctx_ret, v_ret) = toArray(h, ctx, cfg)
    ...
    Map(
      ("jQuery.prototype.get" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h1, ctx1, v1) =
            if (arg1._1._1 </ UndefBot || arg1._1._2 </ NullBot)
              toArray_pre(h, ctx, cfg, PureLocalLoc)
            else
              (HeapBot, ContextBot, ValueBot)
      ...
      ("jQuery.prototype.toArray" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h_ret, ctx_ret, v_ret) = toArray_pre(h, ctx, cfg, PureLocalLoc)
    ...
    Map(
      ("jQuery.prototype.get" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val LP1 =
            if (arg1._1._1 </ UndefBot || arg1._1._2 </ NullBot)
              toArray_def(h, ctx, cfg, fun, args)
            else
              LPBot
          LP1
        })),
  ...
  def getUseMap(): Map[String, AccessFun] = {
    Map(
      ...
      ("jQuery.prototype.toArray" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr) => {
          toArray_use(h, ctx, cfg, fun, args)
        }))
    )
  }
  ...
  private def toArray_def(h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr): LPSet = {
    ...
    val LP2 = lset_this.foldLeft(LPBot)((lpset, l) => {
      ...
      val LP2_2 = AbsNumber.concretize(n_len) match {
        case Some(n) =>
  ...
  private def toArray_use(h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr): LPSet = {
    ...
    val LP4 = lset_this.foldLeft(LPBot)((lpset, l) => {
      ...
      val LP4_2 = AbsNumber.concretize(n_len) match {
        case Some(n) =>
          (0 until n.toInt).foldLeft(LPBot)((lps, i) =>
