...
object HTMLHeadingElement {
  ...
  val loc_proto = newLoc("HTMLHeadingProto")
  
  val F = BoolFalse
  val T = BoolTrue
  
  def init(map: HeapMap) : HeapMap = {
    val loc_cons = newLoc("HTMLHeadingConst")
    ...
    val global_object = map(GlobalLoc).update(AbsString.alpha("HTMLHeadingElement"), 
  ...
  def instantiate(map: HeapMap, node: Node) : (HeapMap, Loc) = {
    val loc_instance = newLoc("HTMLHeadingInstance")
    // Instance Object
