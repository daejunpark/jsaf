...
object HTMLTitleElement {
  ...
  val loc_proto = newLoc("HTMLTitleProto")
  
  val F = BoolFalse
  val T = BoolTrue
  
  def init(map: HeapMap) : HeapMap = {
    val loc_cons = newLoc("HTMLTitleConst")
    ...
    val obj_proto = ObjEmpty.
      ...
      update(OtherStrSingle("@proto"),    PropValue(ObjectValue(HTMLElement.get_prototype, F, F, F)))
       val global_object = map(GlobalLoc).update(AbsString.alpha("HTMLTitleElement"), 
  ...
  def instantiate(map: HeapMap, node: Node) : (HeapMap, Loc) = {
    val loc_instance = newLoc("HTMLTitleInstance")
