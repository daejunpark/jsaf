...
object TIZENseService extends Tizen {
  ...
  val loc_reader: Loc = newPreDefLoc("Reader", Old)
  val loc_readerarr: Loc = newPreDefLoc("ReaderArr", Old)
  val loc_session: Loc = newPreDefLoc("Session", Old)
  val loc_channel: Loc = newPreDefLoc("Channel", Old)
    ...
    Map(
        ("tizen.Session.openBasicChannel" -> (
          (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
            ...
            val (h_5, es_4) = n_arglen match {
              case UIntSingle(n) if n == 2 => (h_4, TizenHelper.TizenExceptionBot)
    ...
    Map(
        ...
        ("tizen.Channel.transmit" -> (
          (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
            ...
            val (h_7, es_4) = n_arglen match {
              case UIntSingle(n) if n == 2 => (h_6, TizenHelper.TizenExceptionBot)
              case UIntSingle(n) if n >= 3 =>
