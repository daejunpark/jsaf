...
object HTMLDocument {
  ...
  val loc_instance = newLoc("DOMHTMLDocumentInstance")
  val loc_proto = newLoc("DOMHTMLDocumentProto")
  
  val F = BoolFalse
  val T = BoolTrue
  
  def init(map: HeapMap) : HeapMap = {
    val loc_cons = newLoc("DOMHTMLDocumentConst")
    ...
    val global_object = map(GlobalLoc).update(AbsString.alpha("HTMLHtmlDocument"), 
                                       PropValue(ObjectValue(loc_cons, T, F, T))).   
                                       update(AbsString.alpha("document"), 
                                       PropValue(ObjectValue(loc_instance, T, F, T)))   
