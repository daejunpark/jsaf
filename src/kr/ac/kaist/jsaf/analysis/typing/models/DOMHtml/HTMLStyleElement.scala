...
object HTMLStyleElement {
  ...
  val loc_proto = newLoc("HTMLStyleProto")
  
  val F = BoolFalse
  val T = BoolTrue
  
  def init(map: HeapMap) : HeapMap = {
    val loc_cons = newLoc("HTMLStyleConst")
    ...
    val global_object = map(GlobalLoc).update(AbsString.alpha("HTMLStyleElement"), 
  ...
  def instantiate(map: HeapMap, node: Node) : (HeapMap, Loc) = {
    val loc_instance = newLoc("HTMLStyleInstance")
