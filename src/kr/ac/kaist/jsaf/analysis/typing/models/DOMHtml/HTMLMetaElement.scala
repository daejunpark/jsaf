...
object HTMLMetaElement {
  ...
  val loc_proto = newLoc("HTMLMetaProto")
  
  val F = BoolFalse
  val T = BoolTrue
  
  def init(map: HeapMap) : HeapMap = {
    val loc_cons = newLoc("HTMLMetaConst")
    ...
    val global_object = map(GlobalLoc).update(AbsString.alpha("HTMLMetaElement"), 
  ...
  def instantiate(map: HeapMap, node: Node) : (HeapMap, Loc) = {
    val loc_instance = newLoc("HTMLMetaInstance")
    ...
    // DOM Level 1
      update(OtherStrSingle("content"),   PropValue(ObjectValue(AbsString.alpha(elementnode.getAttribute("content")), T, T, T))).
      update(OtherStrSingle("httpEquiv"), PropValue(ObjectValue(AbsString.alpha(elementnode.getAttribute("httpEquiv")), T, T, T))).
      update(OtherStrSingle("name"),      PropValue(ObjectValue(AbsString.alpha(elementnode.getAttribute("name")), T, T, T))).
      update(OtherStrSingle("scheme"),    PropValue(ObjectValue(AbsString.alpha(elementnode.getAttribute("scheme")), T, T, T)))
