...
object JQuery extends ModelData {
  ...
  private val prop_easing: List[(String, AbsProperty)] = List(
    ...
    ("@proto",      AbsConstValue(PropValue(ObjectValue(ObjProtoLoc, F, F, F)))),
    ("@extensible", AbsConstValue(PropValue(BoolTrue))),
    ("linear",      AbsInternalFunc("jQuery.easing.linear")),
    ("swing",       AbsInternalFunc("jQuery.easing.swing"))
  )

  def getInitList(): List[(Loc, List[(String, AbsProperty)])] = List(
    (ConstLoc, prop_const), (ProtoLoc, prop_proto), (GlobalLoc, prop_global),
    (RootJQLoc, prop_rootjq), (EasingLoc, prop_easing)
  )
    ...
    Map(
      "jQuery" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val l_tag = addrToLoc(addr1, Recent)
          val l_child = addrToLoc(addr2, Recent)
      ...
      ("jQuery.easing.swing" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          val v_arg = getArgValue(h, ctx, args, "0")
          val n_num = Helper.toNumber(Helper.toPrimitive(v_arg))
          val pi = 3.1415926535897932
          val v_ret = n_num match {
            case NumBot =>  Value(NumBot)
            case NaN|Infinity|PosInf|NegInf =>  Value(NaN)
            case UIntSingle(n) =>  Value(AbsNumber.alpha(scala.math.cos(n*pi)/2))
            case NUIntSingle(n) =>  Value(AbsNumber.alpha(scala.math.cos(n*pi)/2))
            case _ =>  Value(NumTop)
          }
