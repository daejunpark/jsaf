...
object TIZENMessageAttachment extends Tizen {
  ...
  private val prop_cons: List[(String, AbsProperty)] = List(
    ...
    ("@construct",               AbsInternalFunc("TIZENtizen.TIZENMessageAttachment.constructor")),
    ("@hasinstance", AbsConstValue(PropValue(Value(NullTop)))),
    ("prototype", AbsConstValue(PropValue(ObjectValue(Value(loc_proto), F, F, F)))),
    ("id", AbsConstValue(PropValue(Value(UndefTop)))),
    ("messageId", AbsConstValue(PropValue(Value(UndefTop)))),
    ("mimeType", AbsConstValue(PropValue(Value(UndefTop)))),
    ("filePath", AbsConstValue(PropValue(Value(UndefTop))))
  )
  ...
  override def getSemanticMap(): Map[String, SemanticFun] = {
    Map(
/*      ("TIZENtizen.TIZENMessageAttachment.constructor" -> ())  */
    )
  }
