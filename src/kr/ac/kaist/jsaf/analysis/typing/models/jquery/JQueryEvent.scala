...
object JQueryEvent extends ModelData {
  ...
  def getSemanticMap(): Map[String, SemanticFun] = {
    val m = Map(
      ...
      ("jQuery.prototype.ready" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          if (!lset_handler.isEmpty && !lset_target.isEmpty) {
            val h1 = JQueryHelper.addJQueryEvent(h, Value(lset_target), AbsString.alpha("DOMContentLoaded"), Value(lset_handler), ValueBot, ValueBot)
    ...
    List("blur", "change", "click", "dbclick", "error", "focus", "focusin", "focusout",
      ...
      _m + ("jQuery.prototype." + name -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          addEvent(h, ctx, he, ctxe, getArgValue(h, ctx, args, "0"), getArgValue(h, ctx, args, "1"), name)
        }))
    )
  }
  ...
  private def addEvent(h: Heap, ctx: Context, he: Heap, ctxe: Context,
