    ...
    Map(
      ...
      ("tizen.SQLDataControlConsumer.update" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h_3, es_4) = n_arglen match {
            case UIntSingle(n) if n == 3 => (h_2, TizenHelper.TizenExceptionBot)
            case UIntSingle(n) if n == 4 =>
              val v_4 = getArgValue(h_2, ctx_2, args, "3")
              val es1 =
                if (v_4._2.exists((l) => Helper.IsCallable(h_2, l) <= F))
    ...
    Map(
      ...
      ("tizen.MappedDataControlConsumer.getValue" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h_4, es_3) = n_arglen match {
            case UIntSingle(n) if n == 2 => (h_3, TizenHelper.TizenExceptionBot)
            case UIntSingle(n) if n == 3 =>
      ...
      ("tizen.MappedDataControlConsumer.updateValue" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h_3, es_5) = n_arglen match {
            case UIntSingle(n) if n == 4 => (h_2, TizenHelper.TizenExceptionBot)
            case UIntSingle(n) if n == 5 =>
              val v_5 = getArgValue(h_2, ctx_2, args, "4")
              val es1 =
                if (v_5._2.exists((l) => Helper.IsCallable(h_2, l) <= F))
