  ...
  def builtinDate(sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG,
    ...
    fun match {
      ...
      case "Date.constructor" => {
        ...
        val pv_1 = Helper.toPrimitive(getArgValue(h, ctx, "0"))
        val n_prim_3 =
          if (pv_1._5 </ StrBot)
            //parse
            NumTop
      ...
      case "Date.prototype.setTime" => {
        ...
        val h_1 = lset_this.foldLeft(HeapBot)((_h, l) =>
          ...
          ((Helper.ReturnStore(h, time), ctx), (he, ctxe))
      }
      case "Date.prototype.setMilliseconds" |
        ...
        val h_1 = lset_this.foldLeft(HeapBot)((_h, l) =>
          _h + h.update(l, h(l).update("@primitive", PropValue(Value(NumTop)))))
        if (!(h_1 <= HeapBot))
