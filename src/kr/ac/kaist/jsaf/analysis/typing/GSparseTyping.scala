...
class GSparseTyping(_cfg: CFG, quiet: Boolean) extends TypingInterface {
  def cfg = _cfg
  ...
  override def analyze(model: BuiltinModel, duset: DUSet): Unit = {
    ...
    val fixpoint = new GSparseFixpoint(cfg, worklist, inTable, quiet)
  ...
  def checkResult() = {
    ...
    val state = readTable(((cfg.getGlobalFId, LExit), CallContext.globalCallContext))
    val heap = state._1
    val map: Map[String, (PropValue, Absent)] =
      try {
        ...
        case _ =>
          System.out.println("Global object is not found at program exit node")
          HashMap()
      }
    val RESULT = "__result"
    val EXPECT = "__expect"
    ...
    for ((prop, pvalue) <- map) {
      try {
        ...
        case _ => System.out.println("Invalid result/expect variable found: " + prop.toString)
      }
    }
    ...
    for ((index, result) <- resultMap.toSeq.sortBy(_._1)) {
      expectMap.get(index) match {
        case None =>
          System.out.println("No corresponding expect variable is detected for " +
            RESULT +
            index.toString)
        case Some(expect) =>
          ...
          if (!success) {
            ...
            sb.append("}")
            System.out.println("Result is not sound: "+sb.toString)
          }
      }
    }
  }
  ...
  override def getFuncNameByLoc(state:State, loc: Loc): Set[String] = {
    val h = state._1
    AbsString.concretize(h(loc)("@class")._1._2._1._5) match {
