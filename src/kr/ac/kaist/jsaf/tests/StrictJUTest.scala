...
object StrictJUTest {
  ...
  val STRICT_FAIL_TESTS_DIR = "tests/strict_tests"

  def main(args: String*) = junit.textui.TestRunner.run(suite)

  def suite() = {
    val suite = new TestSuite("Test all .js files in 'tests/strict_tests.")
    val failsOnly = true // false if we want to print out the test results
    //$JUnit-BEGIN$
    suite.addTest(FileTests.compilerSuite(STRICT_FAIL_TESTS_DIR, failsOnly, false))
    //$JUnit-END$
    suite
  }
}
